...
一行一件。數量可以寫 `x2` 或者 `2 件`，亦可以上載 Excel。揀完超市撳「格價」。

![貼購物清單，撳格價](docs/usage-1-list.jpg)

結果分三種買法：一間買晒、逐件最平、分兩間買。有「買 2 件」優惠會喺下面提示，加件數之後最平舖可能會轉。

![三種買法同買多D慳多D](docs/usage-2-results.jpg)

自動對到嘅貨可以喺卡片換容量，或者撳「改搜尋」話唔係呢件。

![對到貨之後可以改搜尋](docs/usage-4-matched.jpg)

### 消委會冇呢件

格價資訊通冇貨（例如「玉泉梳打水」）會入「確認一下呢幾件」，自動再搜 HKTVmall／惠康／Price.com.hk。有結果就揀；網站擋自動抽價就開出去睇，再用下面表單填返價錢入清單。

![消委會冇貨，搜其他來源同自己填價](docs/usage-3-not-found.jpg)

### 唔係你要嗰件 → 改搜尋

對錯品牌、容量、口味：卡片撳「改搜尋」，改關鍵字再撳「再搵」。下面會由平至貴列出消委會貨表，仍然唔啱可以搜其他來源，或者跳過。

![改關鍵字再搵，揀返啱嘅貨](docs/usage-6-research.jpg)
...
手機 GitHub 唔會顯示 mermaid，流程用呢張圖：

![模糊搜尋流程：正規化、品名先於容量、品類閘、再決定自動揀定確認](docs/fuzzy-search-flow.jpg)
